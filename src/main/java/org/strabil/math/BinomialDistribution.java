...
public class BinomialDistribution implements Ops.IntToDouble {
    ...
    public BinomialDistribution(final double p, final int n) {
        DoTest.require(p >= 0 && p <= 1.0 , INVALID_PROBABILITY); // QA:[RG]::verified
        this.nExp = n;
        if (p == 0.0) {
    ...
    private double binomialCoefficientLn(final int n, final int k) {
        DoTest.require(n >= 0 , "n < 0 not allowed"); // QA:[RG]::verified // TODO: message
        DoTest.require(k >= 0 , "k < 0 not allowed"); // QA:[RG]::verified // TODO: message
        DoTest.require(n >= k , "n < k not allowed"); // QA:[RG]::verified // TODO: message
    ...
    static public /*@Real*/ double PeizerPrattMethod2Inversion(final double z, final long n) {
    	...
    	DoTest.require(n%2==1,"n must be an odd number"); // TODO: message
